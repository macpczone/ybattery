...
void *thread1(void * arg)
{
	...
	{
		procflag=True;
		if((curfp=fopen("/sys/class/power_supply/BAT1/charge_now","r"))==NULL)		{
			if(openproc())
				return "err";
		}else if((alarmfp=fopen("/sys/class/power_supply/BAT1/alarm","r"))==NULL)
		{
			fclose(curfp);
			if(openproc())
				return "err";
		}else if ((maxfp=fopen("/sys/class/power_supply/BAT1/charge_full","r"))==NULL)
		{
			fclose(alarmfp);
			if(openproc())
				return "err";
		}else if((acfp=fopen("/sys/class/power_supply/AC/online","r"))==NULL)
		{
			fclose(maxfp);
			if(openproc())
				return "err";
